...
CREATE TABLE IF NOT EXISTS `stock-market-charting`.`IPO_details` (
  ...
  `ipo_company_name` VARCHAR(255) NULL,
  `ipo_stock_exchage` VARCHAR(255) NULL,
  `ipo_price_per_share` BIGINT(10) NULL,
  `ipo_total_no_of_share` BIGINT(10) NULL,
  `ipo_open_date` DATETIME NULL,
  `ipo_remarks` VARCHAR(255) NULL,
  `ipo_active` boolean NULL,
  PRIMARY KEY (`ipo_id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `stock-market-charting`.`company` (
  ...
  `co_ceo` VARCHAR(255) NULL,
  `co_board_of_directors` VARCHAR(255) NULL,
  `co_brief_writeup` VARCHAR(255) NULL,
  `co_active` boolean NULL,
  `co_ipo_id` BIGINT(10) NOT NULL,
  `co_stock_code` BIGINT(10) NULL,
  PRIMARY KEY (`co_id`, `co_sc_id`, `co_ipo_id`),
...
CREATE TABLE IF NOT EXISTS `stock-market-charting`.`stock_price` (
  ...
  `sp_company_code` VARCHAR(45) NOT NULL,
  `sp_stock_exchange` VARCHAR(255) NULL,
  `sp_current_price` BIGINT(10) NULL,
  `sp_date` DATE NULL,
  `sp_time` TIME NULL,
  `sp_co_id` BIGINT(10) NOT NULL,
  PRIMARY KEY (`sp_id`, `sp_co_id`),
  UNIQUE INDEX `sp_company_code_UNIQUE` (`sp_company_code` ASC),
  INDEX `fk_stock_price_company1_idx` (`sp_co_id` ASC),
  CONSTRAINT `fk_stock_price_company1`
